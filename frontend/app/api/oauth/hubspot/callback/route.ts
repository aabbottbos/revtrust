...
export async function GET(request: NextRequest) {
  ...
  // If there's an error from HubSpot, redirect to error page
  if (error) {
  ...
  try {
    ...
    const backendUrl = `${process.env.NEXT_PUBLIC_API_URL}/api/oauth/hubspot/callback?code=${encodeURIComponent(code)}&state=${encodeURIComponent(state)}`
    ...
    if (response.ok) {
      return NextResponse.redirect(new URL('/crm?success=hubspot', request.url))
    }
